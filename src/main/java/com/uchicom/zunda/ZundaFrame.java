...
public class ZundaFrame extends JFrame {
  ...
  private JComboBox<String> codeTypeComboBox =
      new JComboBox<String>(ResourceUtil.getStrings(Constants.CHARSETS));
  ...
  private void initComponents() {
    ...
    northPanel.add(new JLabel(ResourceUtil.getString(Constants.LABEL_STRING)), BorderLayout.WEST);
    northPanel.add(new JLabel(ResourceUtil.getString(Constants.LABEL_HEX)), BorderLayout.EAST);
  ...
  public void convertHex2String() {
    ...
    if (hex.length() % 2 == 0) {
      ...
      stringArea.setText(
          new String(bytes, Charset.forName((String) codeTypeComboBox.getSelectedItem())));
    } else {
      stringArea.setBackground(Color.red);
    }
  }
  ...
  public void convertString2Hex() {
    ...
    byte[] bytes = string.getBytes(Charset.forName((String) codeTypeComboBox.getSelectedItem()));
